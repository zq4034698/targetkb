...
export default function ExactSizeLanding({ size, query, title, description, uses, guide, relatedGuide, faqs }: LandingProps) {
  ...
  return <main className="seo-page">
    ...
    <section className="seo-content"><div className="seo-content-inner"><h2>How to compress an image to {size}</h2><p>{guide}</p><ol><li>Open the TargetKB compressor with the {size} limit already selected.</li><li>Upload one image or up to 10 images at once.</li><li>Download the optimized JPG file once it is ready to upload.</li></ol>{relatedGuide && <p><a href={relatedGuide.href}>{relatedGuide.label}</a></p>}<h3>Why use an exact-size compressor?</h3><p>Many upload forms reject files for being only slightly too large. TargetKB works backwards from your requested limit, automatically balancing image dimensions and JPEG quality instead of asking you to guess a quality percentage.</p></div></section>
